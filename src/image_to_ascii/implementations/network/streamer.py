...
async def producer():
    ...
    while True:
        ...
        step = max(4, w // 20)
        p0 = np.array(
            [[x, y] for y in range(0, h, step) for x in range(0, w, step)], np.float32
        ).reshape(-1, 1, 2)
        ...
        for a, b, ok_ in zip(p0.reshape(-1, 2), p1.reshape(-1, 2), st.reshape(-1)):
            if ok_:
                ...
                if 0 <= int(b[0]) < w and 0 <= int(b[1]) < h:
                    flow[int(b[1]), int(b[0])] = max(
                        flow[int(b[1]), int(b[0])], (dx * dx + dy * dy) ** 0.5
                    )
        ...
        if changed:
            await broadcast(
                json.dumps(
                    {
                        "type": "patch",
                        "changes": [{"row": r, "line": l} for r, l in changed],
                    }
                )
            )
